...
void loop(){
  ...
  if (rf12_recvDone() && rf12_crc == 0) {
     ...
     //  Serial.println((int)rf12_data[0]);
     // Serial.println((int)rf12_data[1]);
     Serial.write(rf12_data[0]) ;
    Serial.write(rf12_data[1]);
     /*  addressPIR = rf12_data[0];  
      recsignal = rf12_data[1];   
      check = rf12_data[2]; 
    Serial.write(rf12_data[0]) ;
    Serial.write(rf12_data[1]);
    Serial.println(addressPIR);*/
  }
  //------------------------ recuperation serial------------------
 ...
 //  Serial.println( indata[2]);
  ...
  if (needToSend && rf12_canSend()) {
      ...
      // Serial.println("sending ledstrip RGB values...");
        rf12_sendStart((RF12_HDR_ACK | RF12_HDR_DST | address),byteArray, 4, 1);
      /*  Serial.println("");
        Serial.println();
        Serial.println("");
